...
export function Showcase({ steps }: { steps: ShowcaseStep[] }) {
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    if (pinned) {
      ...
      const HOLD = 0.1;
      const EXIT = 0.4; // уход текста завершается к sub=0.4 — до входа заголовка (0.42)
      // Стадии входа активного шага (по под-прогрессу sub 0..1). Раскрытие
      // завершается уже к sub≈0.74 → дальше (0.74..1.0) шаг стоит СОБРАННЫМ с видимой
